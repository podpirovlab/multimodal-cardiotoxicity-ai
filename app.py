...
def predict_cardio(age, sex, chemo_status):
    """Синтетический демо-инференс.

    ВАЖНО: возраст и пол пациента здесь используются только для отображения
    в отчёте — сигнал ЭКГ синтетический, а итоговая вероятность привязана
    к выбранному сценарию (`chemo_status`), а не к реальному обученному
    классификатору. Это учебная демонстрация пайплайна обработки сигнала,
    а не валидированный диагностический инструмент (см. раздел
    "Limitations" в README).
    """
    fs = 500
...
demo = gr.Interface(
    ...
    description=(
        "Исследовательский прототип для раннего выявления субклинической "
        "антрациклиновой кардиотоксичности по сигналу ЭКГ. "
        "⚠️ Учебная демонстрация пайплайна на синтетических данных — "
        "не является медицинским изделием и не предназначена для клинических решений."
    ),
)

if __name__ == "__main__":
    # server_name="0.0.0.0" необходим для работы в контейнере (Docker/Railway),
    # где приложение должно принимать соединения извне localhost.
