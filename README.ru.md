...
**[Попробовать инструмент →](https://podpirovlab.github.io/multimodal-cardiotoxicity-ai/ru.html)** · [English version](README.md) · [План развития](ROADMAP.ru.md)
...
**[Проверьте на реальной ЭКГ](#проверьте-на-реальной-экг)** — загрузка EDF/CSV, данные PhysioNet, контекст пациента, экспорт FHIR, анатомия отведений
...
## Проверьте на реальной ЭКГ

[Инструмент](https://podpirovlab.github.io/multimodal-cardiotoxicity-ai/ru.html) считает всё, что описано ниже, прямо в браузере — загруженный файл никуда не уходит и не попадает ни на один сервер.

**Что нужно:** одно отведение ЭКГ длиной от ~2 минут. Спектральному методу нужно 128 ударов ([§4.4](#44-спектральный-метод-анализа-альтернации-зубца-t)); в обычной 10-секундной клинической ЭКГ их около 12 — она загрузится, но инструмент честно скажет, что данных мало для надёжной оценки альтернации.

**Форматы файлов:**
- **EDF / EDF+** — читается прямо в браузере (`assets/js/edf.js`), без сервера. Если в файле несколько каналов, после загрузки появится список — выберите, какой из них ЭКГ.
- **CSV / TXT** — один столбец чисел, в мВ или мкВ, частоту дискретизации указываете вручную.

**Где взять реальную запись на PhysioNet:**
- [T-Wave Alternans Challenge Database](https://physionet.org/content/twadb/1.0.0/) — записи холтеровской длины (реальные и смоделированные), сделанные специально для проверки алгоритмов TWA.
- [PTB-XL](https://physionet.org/content/ptb-xl/1.0.3/) — 21 799 реальных клинических ЭКГ (используются для обучения, [§5.4](#54-данные-ptb-xl)), но каждая всего по 10 секунд — для TWA самой по себе мало. Зато удобно проверить, что инструмент корректно работает на настоящей форме сигнала.

Обе базы хранятся в формате WFDB (`.dat` + `.hea`), который браузер напрямую не читает — сначала переведите одно отведение в CSV:
...
Затем загрузите `ecg.csv` и укажите частоту дискретизации, которую выведет скрипт.

**Контекст пациента и экспорт FHIR.** Возраст, пол и кумулятивная доза антрациклинов рядом с загрузкой — необязательные поля. Они не меняют цифры TWA (анализ смотрит только на саму ЭКГ) — они нужны, чтобы кнопка «Экспортировать FHIR-отчёт» могла приложить их к результату в виде скачиваемого `DiagnosticReport` (та же структура, что в `cardioonco/fhir.py`, [§6.3](#63-hl7-fhir-r4)). Поле дозы также показывает, где это число находится на кривой доза–риск из [§2.1](#21-масштаб-проблемы) — статистика по группе пациентов, а не прогноз для конкретного человека.

**Какое отведение видит какую стенку.** 12 отведений ЭКГ смотрят на сердце с 12 направлений:

| Стенка | Отведения, которые видят её лучше всего |
|---|---|
| Перегородка | V1, V2 |
...
aVR не привязано к конкретной стенке и в схему не входит — это стандартная практика. Поражение антрациклинами обычно диффузное, а не очаговое, поэтому эта таблица — про анатомию ЭКГ, а не про то, где именно повреждён миокард у конкретного пациента: инструмент ничего не локализует.
